...
export class FirebaseService {
  ...
  async registerEvent(eventTitle: string, eventDesc: string, maxParticipants: number, eventLocation: string, eventDate: string, eventTime: string, image: any): Promise<void> {
    ...
    if (!this.othersService.checkIfFileTypeIsCorrect(image)) {
      ...
      const eventDocRef = await this.firestore.collection(this.eventsPath).add({ eventTitle, eventDesc, maxParticipants, eventLocation, eventDate, eventTime, ownerUid });
  ...
  updateEvent(newEventTitle: string, newEventDesc: string, newMaxParticipants: number, newEventLocation: string, newEventDate: string, newEventTime: string, eventId: string): Promise<void> {
    return this.firestore.collection(this.eventsPath).doc(eventId).update({ eventTitle: newEventTitle, eventDesc: newEventDesc, maxParticipants: newMaxParticipants, eventLocation: newEventLocation, eventDate: newEventDate, eventTime: newEventTime });
  }
